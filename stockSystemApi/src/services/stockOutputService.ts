...
export class StockOutputService {
    ...
    async output(newStockOutput: any) {
     ...
     const stockOutputId = await this.stockOutputRepository.output(newStockOutput.stockOutput);
        await  this.productsRepository.decrease(newStockOutput.products)
        await  this.outputItemRepository.output(newStockOutput.products ,stockOutputId )
    }
